...
func main() {
    ...
    mod.Registers["id"].Blocks["info"].Read(&p)
    ...
    values = mod.Registers["id"].Blocks["info"].GetReads(rep)
    ...
    mvals := mod.Registers["id"].Blocks["info"].GetMaskedReads("fwrev", rep)
    fmt.Printf("id.info.fwrev values = %x\n", mvals)
    mvals = mod.Registers["id"].Blocks["info"].GetMaskedReads("slot", rep)
