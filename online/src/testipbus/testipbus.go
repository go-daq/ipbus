...
func main() {
    ...
    mod.Registers["id"].Words["magic"].Read(&p)
    mod.Registers["id"].Words["info"].Read(&p)
    //mod.Registers["csr"].Words["ctrl"].Read(&p)
    ...
    values := mod.Registers["id"].Words["magic"].GetReads(rep)
    fmt.Printf("id.magic values = %x\n", values)
    values = mod.Registers["id"].Words["info"].GetReads(rep)
    fmt.Printf("id.info values = %x\n", values)
    ...
    mvals := mod.Registers["id"].Words["info"].GetMaskedReads("fwrev", rep)
    fmt.Printf("id.info.fwrev values = %x\n", mvals)
    mvals = mod.Registers["id"].Words["info"].GetMaskedReads("slot", rep)
    fmt.Printf("id.info.slot values = %x\n", mvals)
